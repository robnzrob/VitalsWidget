...
/// <summary>
/// Linux AMD GPU temp via /sys/class/hwmon (amdgpu).
/// Best effort. Returns false if not found.
/// </summary>
public sealed class LinuxAmdHwmonGpuProvider : IGpuTempProvider
{
    private readonly string? _tempInputPath;

    public LinuxAmdHwmonGpuProvider()
    {
        _tempInputPath = TryFindGpuTempInputPath("amdgpu");
    }
    ...
    private static string? TryFindGpuTempInputPath(string nameContains)
    {
        ...
        try
        {
            ...
            // Most common GPU sensor is temp1_input, but we fall back to any temp*_input
